...
server.get('/', (_, res) => {
    res.status(200).json({ message: 'OK!', users: `localHost:${config.port}/api/v1/users` })
});
...
server.listen(config.port, () => console.log(`Server started at port ${config.port}`+config.db.username+config.db.password +config.db.host));